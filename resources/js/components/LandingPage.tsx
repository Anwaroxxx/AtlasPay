...
export default function LandingPage() {
  ...
  return (
    <div ref={containerRef} className="min-h-screen bg-[#010301] text-white selection:bg-primary/30 overflow-x-hidden font-sans">
        ...
        {mobileMenuOpen && (
          <>
            ...
            <motion.div
              ...
              className="fixed right-0 top-0 bottom-0 z-[201] w-72 bg-[#010301] border-l border-white/10 flex flex-col p-8 gap-8"
            >
              <button
                onClick={() => setMobileMenuOpen(false)}
                className="self-end p-2 rounded-full hover:bg-white/10 transition-colors"
              >
              ...
              <div className="flex flex-col gap-6">
                {['Vault', 'Elite', 'Archive'].map((item) => (
                  <a
                    ...
                    onClick={() => setMobileMenuOpen(false)}
                    className="text-sm uppercase tracking-[0.3em] font-black text-white/60 hover:text-primary transition-colors"
                  >
                    {item}
                  </a>
                ))}
              ...
              <div className="flex flex-col gap-4 mt-auto">
                <Link
                  href="/login"
                  onClick={() => setMobileMenuOpen(false)}
                  className="text-center text-sm uppercase tracking-widest font-black text-white/40 hover:text-white transition-colors py-3 border border-white/10 rounded-full"
                >
                  Access
                </Link>
                <Link href="/register" onClick={() => setMobileMenuOpen(false)}>
                  <Button className="w-full bg-primary hover:bg-success text-white font-black px-8 h-12 rounded-full uppercase tracking-widest text-[10px] border-none">
                    Get Started
                  </Button>
                </Link>
              </div>
      ...
      <div className="fixed top-0 left-0 right-0 z-[100] flex justify-center py-4 md:py-8 px-4 md:px-6 transition-all duration-500">
        <motion.nav 
          animate={{ 
          ...
          className={`max-w-screen-2xl flex items-center justify-between backdrop-blur-2xl border transition-all duration-500 ${
            isScrolled 
            ? 'bg-white/[0.03] border-white/10 rounded-full px-6 md:px-8 py-3 shadow-[0_20px_50px_rgba(0,0,0,0.5)]' 
            : 'bg-transparent border-transparent px-4 md:px-8 py-4 w-full'
          }`}
        >
          <div className="flex items-center gap-4">
            <Link href={home()} className="flex items-center">
              <img 
                ...
                alt="AtlasPay Logo" 
                className={`object-contain transition-all ${isScrolled ? 'h-10 md:h-12' : 'h-16 md:h-24'}`}
              />
            </Link>
          </div>

          {/* Desktop nav links */}
          <div className={`hidden md:flex items-center gap-8 ${isScrolled ? 'mx-8' : ''}`}>
            {['Vault', 'Elite', 'Archive'].map((item) => (
              <a 
                key={item} 
                href={`#${item.toLowerCase()}`} 
      ...
      <main className="relative z-10">
        ...
        <section className="relative min-h-screen flex items-center justify-center px-4 md:px-6 pt-28 md:pt-32 pb-16 md:pb-20">
          <div className="max-w-screen-2xl mx-auto w-full grid lg:grid-cols-12 gap-8 md:gap-16 items-center">
            <div className="lg:col-span-7 space-y-8 md:space-y-12">
              ...
              >
                ...
                <h1 className="text-5xl sm:text-6xl md:text-8xl lg:text-9xl font-display font-black leading-[0.82] tracking-tighter uppercase">
                  Wealth <br />
              ...
              >
                <Link href="/register">
                  <Button className="h-14 md:h-20 px-10 md:px-16 bg-white text-black hover:bg-primary hover:text-white rounded-full text-xs font-black uppercase tracking-[0.3em] transition-all hover:scale-[1.05] shadow-[0_20px_80px_rgba(255,255,255,0.1)] border-none w-full sm:w-auto">
                    Begin Journey
                  </Button>
                </Link>
            ...
            <div className="lg:col-span-5 relative group">
              ...
              >
                ...
                <motion.img 
                  ...
                  alt="Platinum Card" 
                  className="relative w-full h-auto drop-shadow-[0_80px_120px_rgba(0,0,0,1)] rotate-[-12deg] group-hover:rotate-0 transition-all duration-1000 ease-out cursor-pointer max-w-sm mx-auto lg:max-w-none"
                />
              </motion.div>
            </div>
          </div>
        ...
        <section className="py-16 md:py-24 border-y border-white/5 relative">
          ...
          <div className="max-w-screen-2xl mx-auto px-4 md:px-12 relative z-10">
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6 p-4 md:p-8">
               {[
                 ...
                 <div key={stat.label} className="space-y-3 md:space-y-4 group cursor-help">
                    <div className="flex items-center gap-2 md:gap-3 text-primary/40 group-hover:text-primary transition-colors">
                       <stat.icon className="w-4 h-4 md:w-5 md:h-5 shrink-0" />
                       <span className="text-[8px] md:text-[10px] font-black uppercase tracking-[0.2em] md:tracking-[0.4em] truncate">{stat.label}</span>
                    </div>
                    <div className="text-3xl md:text-5xl font-display font-black tracking-tighter group-hover:text-glow transition-all">{stat.val}</div>
                 </div>
               ))}
            </div>
          </div>
        ...
        {/* --- Card Swap Section --- */}
        <section id="vault" className="py-24 md:py-48 px-4 md:px-12 relative">
           <div className="max-w-screen-2xl mx-auto grid lg:grid-cols-12 gap-12 md:gap-32 items-center">
              <div className="lg:col-span-6 space-y-10 md:space-y-16">
                 <div className="space-y-6 md:space-y-8">
                    ...
                    <h2 className="text-5xl md:text-6xl lg:text-8xl font-display font-black tracking-tighter uppercase leading-[0.9]">
                      Curated <br /> <span className="italic text-primary">Destinies.</span>
                    </h2>
                    <p className="text-white/40 text-base md:text-xl leading-relaxed max-w-lg font-medium">
                 ...
                 <div className="grid gap-4 md:gap-6">
                    {features.map((f, i) => (
                      ...
                      >
                         <div className="h-12 w-12 md:h-16 md:w-16 shrink-0 rounded-[1.5rem] border border-white/5 bg-white/[0.02] flex items-center justify-center group-hover:border-primary/50 group-hover:bg-primary/5 transition-all">
                            <f.icon className="w-5 h-5 md:w-7 md:h-7 text-primary" />
                         </div>
                         <div className="space-y-1 md:space-y-2">
                            <h4 className="text-base md:text-xl font-black uppercase tracking-tight">{f.title}</h4>
                            <p className="text-sm md:text-base text-white/30 font-medium leading-relaxed">{f.description}</p>
                         </div>
                      </motion.div>
                    ))}
                 </div>
              ...
              <div className="lg:col-span-6 flex justify-center lg:justify-end min-h-[350px] md:min-h-[650px] relative">
        ...
        {/* --- Final Statement --- */}
        <section className="py-24 md:py-60 px-4 md:px-6 relative overflow-hidden">
           ...
           <div className="max-w-5xl mx-auto text-center space-y-10 md:space-y-16">
              <div className="space-y-4 md:space-y-6">
                 <h2 className="text-5xl sm:text-6xl md:text-7xl lg:text-9xl font-display font-black tracking-tighter uppercase leading-[0.85]">
                   Secure your <br /> <span className="text-primary italic">Sovereignty.</span>
                 </h2>
                 <p className="text-base md:text-xl text-white/30 font-medium tracking-wide px-4">Join the exclusive network of Moroccan elite banking.</p>
              </div>
              
              <div className="flex flex-col sm:flex-row gap-6 md:gap-8 justify-center items-center px-4">
                 <Link href="/register" className="w-full sm:w-auto">
                   <Button className="w-full sm:w-auto h-14 md:h-24 px-10 md:px-20 bg-primary text-white rounded-full text-sm font-black uppercase tracking-widest shadow-[0_20px_50px_rgba(54,105,75,0.4)] transition-all hover:scale-105 hover:bg-success border-none">
                     Create Free Account
                   </Button>
                 </Link>
        ...
        {/* --- Footer --- */}
        <footer className="py-12 md:py-24 px-4 md:px-6 border-t border-white/5 bg-black/40">
           <div className="max-w-screen-2xl mx-auto flex flex-col md:flex-row justify-between items-center gap-6 md:gap-8 p-4 md:p-8">
              <div className="flex items-center gap-4">
                <img 
                  src="/images/logos/darkmode-Photoroom.png" 
                  alt="AtlasPay Logo" 
              ...
              <div className="flex flex-wrap justify-center gap-4 md:gap-16 text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.4em] text-white/30">
              ...
              <div className="text-[10px] font-black uppercase tracking-[0.3em] md:tracking-[0.5em] text-white/10 text-center">
