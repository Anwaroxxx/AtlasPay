...
import { ArrowDownLeft, ArrowUpRight, Gauge, PiggyBank, Zap, Shield, Target, BrainCircuit, Activity } from "lucide-react";
...
export default function Dashboard({ stats, accounts, recentTransactions, chartData }: Props) {
        ...
        if (typeof window !== 'undefined' && (window as any).Echo) {
            ...
            (window as any).Echo.private(channel)
                .listen('.transaction.created', (e: any) => {
                    ...
                    router.reload({ only: ['stats', 'accounts', 'recentTransactions', 'chartData'] });
                });
    ...
    const totalIn = recentTransactions.filter(t => t.type === 'deposit').reduce((sum, t) => sum + t.amount, 0);
    const totalOut = recentTransactions.filter(t => t.type === 'withdrawal' || t.type === 'transfer').reduce((sum, t) => sum + t.amount, 0);
        ...
        <>
            ...
            >
                ...
                <motion.div variants={item} className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="font-display text-3xl font-black tracking-tighter md:text-5xl uppercase leading-none">
                            Welcome, <span className="text-primary italic">{userName}.</span>
                        </h1>
                        <div className="flex items-center gap-2 text-muted-foreground mt-1">
                            <Shield className="h-3 w-3 text-success" />
                            <span className="text-[10px] font-bold uppercase tracking-widest">All Good</span>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 bg-card/50 backdrop-blur-xl px-4 py-2 rounded-2xl border border-border/50 shadow-soft">
                ...
                <motion.div variants={item} className="grid gap-6 lg:grid-cols-12">
                    <div className="lg:col-span-8">
                        <BalanceCard 
                            ...
                            accountNumber={mainAccount ? mainAccount.account_number : "MA53 ATLS 0009 8821 4477"} 
                        />
                    </div>
