...
import { Cell, Pie, PieChart, ResponsiveContainer, Tooltip, Bar, BarChart, XAxis, YAxis, CartesianGrid, Legend } from "recharts";
    ...
    <div className="mx-auto w-full max-w-6xl px-4 py-6 md:px-6 md:py-8">
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <h1 className="mt-1 font-display text-2xl font-semibold md:text-3xl">Monthly reports</h1>
        </div>
      ...
      <div className="mt-6 grid gap-4 sm:grid-cols-3">
        {[
          { l: "Income", v: "14,820", t: "+18% vs Apr", tone: "success" },
          { l: "Expenses", v: "5,790", t: "-8% vs Apr", tone: "success" },
          { l: "Net saved", v: "9,030", t: "+62% vs Apr", tone: "success" },
        ].map((s) => (
          <div key={s.l} className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-soft)]">
            ...
            <p className="mt-2 font-display text-3xl font-semibold">{s.v} <span className="text-base text-muted-foreground">MAD</span></p>
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-5">
          ...
          <ul className="mt-2 space-y-1.5 text-xs">
            {pie.map((p, i) => (
              <li key={p.name} className="flex items-center justify-between">
                ...
                <span className="tabular-nums text-muted-foreground">{p.value.toLocaleString()}</span>
              </li>
            ))}
          </ul>
        </div>
        ...
        <div className="lg:col-span-3 rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-soft)]">
          <h3 className="font-display text-base font-semibold">In vs Out — last 6 months</h3>
          <div className="mt-4 h-72">
            <ResponsiveContainer>
              <BarChart data={compare} margin={{ top: 10, right: 8, left: -10, bottom: 0 }}>
                ...
                <Bar dataKey="in" name="Income" fill="var(--color-primary)" radius={[6, 6, 0, 0]} maxBarSize={28} />
                <Bar dataKey="out" name="Spending" fill="var(--color-mint)" radius={[6, 6, 0, 0]} maxBarSize={28} />
              </BarChart>
            </ResponsiveContainer>
          </div>
        </div>
