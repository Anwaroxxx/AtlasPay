...
import { QrCode, Camera, Send, Store, Clock, Shield, Copy, Check } from "lucide-react";
  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 md:px-6 md:py-8">
      <header className="mb-6">
        <p className="text-xs uppercase tracking-[0.18em] text-muted-foreground">Instant transfer</p>
        <h1 className="mt-1 font-display text-2xl font-semibold md:text-3xl">QR Payments</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Send, receive, scan or display a merchant code — secured by one-time tokens.
        </p>
      </header>
      ...
      <div className="flex gap-2 overflow-x-auto rounded-2xl border border-border bg-card p-1.5">
        {tabs.map((t) => (
          <button
            ...
            onClick={() => setTab(t.id)}
            className={cn(
              "flex flex-1 min-w-[110px] items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-sm font-medium transition",
              tab === t.id
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-5">
        <div className="lg:col-span-3 rounded-3xl border border-border bg-card p-5 md:p-7 shadow-[var(--shadow-soft)]">
          {tab === "scan" ? (
            <div className="flex flex-col items-center text-center">
              ...
              <p className="mt-5 text-sm text-muted-foreground">Point your camera at any AtlasPay QR code.</p>
            ...
            <>
              ...
              <div className="mt-4">
                ...
                <div className="mt-1 flex items-baseline gap-2">
                  ...
                  />
                  <span className="font-display text-xl font-medium text-muted-foreground">MAD</span>
                </div>
              </div>
              ...
              <div className="mt-6 grid place-items-center rounded-3xl bg-[image:var(--gradient-mint)] p-6">
                <FakeQR />
                <p className="mt-3 font-mono text-xs text-deep">ATLS-{Math.floor(Math.random() * 9000 + 1000)}-{Math.floor(Math.random() * 9000 + 1000)}</p>
              </div>

              <div className="mt-5 flex flex-wrap gap-3">
                <Button className="flex-1">Generate new QR</Button>
                <Button
                  variant="outline"
                  onClick={() => {
                    setCopied(true);
        ...
        <div className="lg:col-span-2 space-y-4">
          <div className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-soft)]">
            ...
            <p className="mt-2 text-xs text-muted-foreground">
              Each QR is a one-time signed token. We score the receiver and flag unusual amounts before you confirm.
            </p>
          ...
          <div className="rounded-2xl border border-border bg-card p-5 shadow-[var(--shadow-soft)]">
            <h3 className="text-sm font-semibold">Recent QR activity</h3>
            <ul className="mt-3 space-y-3 text-sm">
              {[
                { n: "Hanout Si Mohamed", a: -28, t: "2 min ago" },
                ...
                <li key={r.n} className="flex items-center justify-between">
                  ...
                  <span className={r.a > 0 ? "text-success font-semibold" : "font-semibold"}>
                    {r.a > 0 ? "+" : ""}
                    {r.a} MAD
                  </span>
                </li>
...
function FakeQR() {
  ...
  const cells = Array.from({ length: 21 * 21 }, (_, i) => {
    ...
    return finder ? ((x === 0 || x === 6 || y === 0 || y === 6 || (x > 1 && x < 5 && y > 1 && y < 5)) ? 1 : 0) : seed > 3 ? 1 : 0;
  });
  return (
    <div className="grid aspect-square w-56 grid-cols-[repeat(21,1fr)] gap-[2px] rounded-xl bg-card p-3">
      {cells.map((c, i) => (
        <div key={i} className={c ? "bg-deep rounded-[1px]" : "bg-transparent"} />
      ))}
    </div>
  );
}
