...
class AnwarLogic
{
    ...
    public static function getSeasonalNudges($now)
    {
        ...
        if ($month == 3 || $month == 4) {
            $nudges[] = 'Ramadan Cycle: Food & Charity nodes likely to spike (+30%).';
        }
        if ($month == 6 || $month == 7) {
            $nudges[] = 'Eid Al-Adha: Exceptional capital deployment detected for ritual livestock.';
        }
        if ($month == 8) {
            $nudges[] = 'Summer Surge: Leisure and travel sectors show high volatility.';
        }
        if ($month == 9) {
            $nudges[] = 'Educational Reset: Rentrée scolaire detected. Optimize for tuition fees.';
        }
